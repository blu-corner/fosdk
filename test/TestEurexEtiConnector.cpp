...
class MockEurexConnector : public gwcEti<eurexCodec>
{
public:
    MockEurexConnector (logger* log)
        : gwcEti<eurexCodec> (log)
    { }
...
class EurexEtiTestHarness : public Test
{
protected:
    virtual void SetUp()
    {
        ::remove ("eurex.seqno.cache");
        
        mLogger = logService::getLogger ("TEST_EUREX");
        mProps = new properties("gwc", "eurex", "sim");

        mSessionCallbacks = new MockSessionCallbacks();
        mMessageCallbacks = new MockMessageCallbacks();

        mConnector = new MockEurexConnector(mLogger);
    ...
    {
        mTcpDelegate =
                new gwcEtiTcpConnectionDelegate<eurexCodec>(mConnector);
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_CONNECTION_READY_ONLOGGINGON_IS_CALLED)
{
    // setup
    mockInitilizeConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_CONNECTION_READY_WITH_LOGON_REPLY_ON_ADMIN_IS_CALLED)
{
    // setup
    mockInitilizeConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_CONNECTION_READY_WITH_LOGON_REJECT_ON_ERROR_IS_CALLED)
{
    // setup
    mockInitilizeConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_CANNOT_SEND_ORDER_IF_NOT_LOGGED_ON)
{
    // setup
    mockInitilizeConnector ();
    EXPECT_CALL(*mSessionCallbacks, onLoggingOn(_)).Times(1);
...
TEST_F(EurexEtiTestHarness, TEST_THAT_CAN_SEND_ORDER_IF_LOGGED_ON)
{
    // setup
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_ORDER_ACK_CALLBACK_IS_CALLED)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_ORDER_DONE_CALLBACK_IS_CALLED_WITH_ORD_STATUS_FOUR_IN_ORDER_ACK)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_MODIFY_ACK_CALLBACK_IS_CALLED)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_ORDER_DONE_CALLBACK_IS_CALLED_WITH_ORD_STATUS_FOUR_IN_MODIFY_ACK)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_ORDER_REJECTED_CALLBACK_IS_CALLED)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_IMMEDIATE_FILL_ORDER_ACK_AND_ORDER_FILL_CALLBACKS_ARE_CALLED)
{
    // setup
    mockFullInitilizedConnector ();
...
TEST_F(EurexEtiTestHarness, TEST_THAT_ON_ORDER_BOOK_EXECUTION_ON_ORDER_FILL_CALLBACK_IS_CALLED)
{
    // setup
    mockFullInitilizedConnector ();
